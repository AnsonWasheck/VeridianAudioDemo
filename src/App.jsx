...
// ── Global CSS — kills all scroll/bounce for a locked iPad web app ────────────
const GLOBAL_CSS = `
...
// ── useOrientation — uses real WebKit screen.orientation API ─────────────────
...
function Waveform({ recording, light, canvasWidth }) {
  ...
  useEffect(() => {
    ...
    canvas.width  = canvasWidth || IPAD_P_W;
    canvas.height = 200;
  }, [canvasWidth]);
...
// ── Patient card ──────────────────────────────────────────────────────────────
const VITALS = [
  {label:"BP",   value:"124/82"},{label:"HR",   value:"71 bpm"},
  {label:"SpO₂", value:"98%"},   {label:"Temp", value:"36.8°C"},
  {label:"RR",   value:"16/min"},{label:"BMI",  value:"24.1"},
];
...
const PatientCard = memo(function PatientCard({ recording, T, portrait }) {
  ...
  const lblStyle = useMemo(()=>({
    ...
    color:recording?T.labelTxtRec:T.labelTxt, transition:"color 0.6s ease",
  ...
  return (
    <div style={{
      ...
      borderRadius:11, width:portrait?"100%":460,
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"15px 18px",borderBottom:`1px solid ${hidden?T.divider:"transparent"}`,transition:"border-color 0.3s ease"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:34,height:34,borderRadius:7,background:T.iconBox,border:`1px solid ${T.iconBoxBorder}`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 1px 0 rgba(0,0,0,0.4),0 3px 8px rgba(0,0,0,0.18)",flexShrink:0,transition:"background 0.5s ease"}}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={T.iconStroke} strokeWidth="1.7" strokeLinecap="round" style={{transition:"stroke 0.5s ease"}}><circle cx="12" cy="7" r="4"/><path d="M4 21v-1a8 8 0 0 1 16 0v1"/></svg>
          </div>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:7}}>
              <span style={{color:T.nameTxt,fontSize:13.5,fontWeight:500,letterSpacing:"0.01em",transition:"color 0.5s ease"}}>Eleanor Voss</span>
        ...
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          ...
          <button onClick={toggleHidden} style={{borderRadius:5,padding:"5px 10px",fontSize:8,letterSpacing:"0.18em",textTransform:"uppercase",outline:"none",border:`1px solid ${T.hideBtnBorder}`,background:T.hideBtn,color:T.hideBtnTxt,display:"flex",alignItems:"center",gap:5,boxShadow:"0 1px 0 rgba(0,0,0,0.3),0 3px 8px rgba(0,0,0,0.15)",transition:"all 0.3s ease"}}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
              {hidden?<><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></>:<><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></>}
            </svg>
            {hidden?"Show":"Hide"}
          </button>
        </div>
...
const ACT_BTN_BASE = {
  ...
  borderRadius:7, padding:"9px 14px", outline:"none",
...
const ActionButtons = memo(function ActionButtons({ hasRecording, onDelete, onSoap, T }) {
  ...
  const handleSoap = useCallback(()=>{ onSoap(); setSoapDone(true); }, [onSoap]);
  ...
  const Btn = ({ onClick, variant="normal", children, title }) => {
    ...
    return (
      <button title={title} onClick={onClick}
        ...
        style={{...ACT_BTN_BASE,background:v.bg,border:`1px solid ${v.border}`,color:v.txt,boxShadow:T.actShadow,opacity:hover?1:0.92,transform:hover?"translateY(-1px)":"translateY(0)"}}>
        {children}
      </button>
    );
  };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
      <div style={{display:"flex",alignItems:"center",gap:6,maxHeight:showMore?60:0,overflow:"hidden",transition:"max-height 0.32s cubic-bezier(0.4,0,0.2,1), opacity 0.25s ease",opacity:showMore?1:0}}>
        <Btn title="Export as PDF"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>Export PDF</Btn>
        <Btn title="Copy transcript"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>Copy Transcript</Btn>
        <Btn title="Send to EHR"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Send to EHR</Btn>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <Btn variant="danger" onClick={onDelete} title="Delete recording"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>Delete</Btn>
        <Btn onClick={toggleMore} title="More options"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>{showMore?"Less":"More"}</Btn>
        <Btn variant="primary" onClick={handleSoap} title="Generate SOAP note"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>{soapDone?"Regenerate SOAP":"SOAP Note"}</Btn>
      </div>
    </div>
  );
});
...
const RecordButton = memo(function RecordButton({ recording, onToggle, T }) {
  ...
  // Unified pointer handlers — preventDefault stops iOS ghost clicks & scroll
  const onPressStart  = useCallback((e)=>{ e.preventDefault(); setPress(true); }, []);
  const onPressEnd    = useCallback((e)=>{ e.preventDefault(); setPress(false); onToggle(); }, [onToggle]);
  const onPressCancel = useCallback((e)=>{ e.preventDefault(); setPress(false); }, []);

  return (
    // inline-flex wrapper with explicit size so it never stretches inside a flex row
    <div style={{
      position:"relative",
      display:"inline-flex",
      alignItems:"center",
      ...
      <button
        ...
        onTouchStart={onPressStart} onTouchEnd={onPressEnd} onTouchCancel={onPressCancel}
...
export default function App() {
  ...
  // Auto-detects real iPad orientation via WebKit screen.orientation API
  ...
  // Block all touch scroll/bounce events at the document level
  useEffect(()=>{
  ...
  // Keyboard shortcut: O = toggle theme
  useEffect(()=>{
    const h=(e)=>{
  ...
  const themeBtnStyle = useMemo(()=>({
    ...
    transition:"all 0.4s ease", touchAction:"manipulation",
  }), [light, T.topbarSub]);
        ...
        <>
          ...
          <div style={{position:"absolute",bottom:28,left:24,right:24,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingBottom:4}}>
              <ActionButtons hasRecording={hasRecording} onDelete={handleDelete} onSoap={handleSoap} T={T}/>
